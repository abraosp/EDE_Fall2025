...
knitr::kable(metadata, caption = "EPA Air Quality", align = c('r','l'))
```


## Alternative Simpler R Code:


```{r task1-table, echo=FALSE}
library(knitr)

# Create and display the table in one step
data.frame(
  Item = c("Filename", "Date", "Source"),
  Value = c("EPAair_O3_PM25_NC1819_Processed.csv", "2018-2019", "EPA Air Quality System (AQS)")
) %>% 
...
column_names <- c("county", "$\\mu g/m^3$")

# Table 1. Mean Particles (2.5mm)

cat("### Table: Mean Particles (2.5mm)\n")
summary_PM25 %>%
  mutate(mean_PM2.5 = round(mean_PM2.5, 2)) %>%
  knitr::kable(caption = "Lists of Mean Particles (2.5mm)",col.names = column_names
  )


# Table 2. Mean Zone 
cat("### Table: Mean Ozone\n")
summary_Ozone %>%
...
```{r 2018-plot, echo=FALSE, fig.align='left', fig.height=3.5, fig.cap="Ozone Levels by Month - 2018"}
# Records collected in 2018 
epa_data %>%
  filter(year(Date) == 2018) %>%
  drop_na(Ozone) %>%
  ggplot(aes(x = factor(month(Date)), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone (µg/m³)") +
  theme_minimal()
```

```{r 2019-plot, echo=FALSE, fig.align='left', fig.height=3.5, fig.cap="Ozone Levels by Month - 2019"}
# Records collected in 2019 
epa_data %>%
