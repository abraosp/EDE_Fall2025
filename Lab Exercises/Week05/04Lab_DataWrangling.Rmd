...
# dplyr filtering 
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # where depth is equal to zero

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # give everything where lake is equal to Paul and Peter lake. 
...
NTL.phys.data.in.90through99 <- filter(NTL.phys.data, 
...
NTL.phys.data.1990.July <- filter (NTL.phys.data %>%
